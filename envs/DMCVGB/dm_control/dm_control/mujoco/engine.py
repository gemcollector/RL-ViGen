...
class Physics(_control.Physics):
  ...
  def reload_from_xml_string(self, xml_string, assets=None, domain_name=None):
    ...
    if domain_name == 'unitree':
      ...
      os.chdir(current_dir + '/../suite/mujoco_menagerie/unitree_a1')
      new_model = wrapper.MjModel.from_xml_string(xml_string, assets=assets)
      os.chdir(origin_dir)
    elif domain_name == 'franka':
      origin_dir = os.getcwd()
      current_dir = os.path.dirname(__file__)
      os.chdir(current_dir + '/../suite/mujoco_menagerie/franka_emika_panda')
      new_model = wrapper.MjModel.from_xml_string(xml_string, assets=assets)
      os.chdir(origin_dir)
    else:
      new_model = wrapper.MjModel.from_xml_string(xml_string, assets=assets)
