...
def _make_habitat_gym_env(
    ...
    cfg_dict["appearance_id"] = appearance_id
    src_file = os.path.join(f'data/scene_datasets/habitat-test-scenes/{appearance_id}', 'skokloster-castle.glb')
    ...
    if mode == 'test':
        ...
        if cfg_dict["scene_id"] == 0:
            print(' === remain the same lighting with training === ')
            cfg_dict['setting']['light'] = cfg_dict['setting']['light_train']
    else:
        config.habitat.dataset.data_path = cfg_dict['setting']['data_path'][cfg_dict['mode']]
        cfg_dict['setting']['light'] = cfg_dict['setting']['light_train']
    ...
    color_b = random_state.uniform(cfg_dict['setting']['light']['color'][cfg_dict['light']['color']][0],
                                   cfg_dict['setting']['light']['color'][cfg_dict['light']['color']][
                                       1]) * intensity
    if cfg_dict['mode'] == 'train' or cfg_dict["scene_id"] == 0:
        print('=== remain using the same lighting setting with training ===')
        my_scene_lighting_setup = [
            LightInfo(vector=[pos_x, pos_y, pos_z, 1.0], color=[color_r, color_g, color_b], model=LightPositionModel.Global)
        ]
    else:
            my_scene_lighting_setup = [
