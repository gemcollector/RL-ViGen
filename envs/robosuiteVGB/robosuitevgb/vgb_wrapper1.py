...
from SECANT.secant.envs.robosuite.custom_xml import CustomMujocoXML, XMLTextureModder
from SECANT.secant.utils.misc import render_img
from SECANT.secant.envs.robosuite.utils import get_obs_shape_from_dict
from SECANT.secant.envs.robosuite.preset_customization import (
...
class VGBWrapper(gym.core.Env, Wrapper):
    ...
    def _reset_from_xml(self, xml_string):
        ...
        if self.secant_modders["texture"] is not None:
            self.secant_modders["texture"].random_texture_change(mujoco_xml)
    ...
    def step(self, action):
        # self.step_randomization()  # TODO 如果影响texture,可以把这个去掉(只有reset时会改)
